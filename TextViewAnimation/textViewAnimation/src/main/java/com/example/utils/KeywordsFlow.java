...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener{
    ...
    /** 存储显示的关键字。 */  
    private List<PushNews> vecKeywords;
    ...
    private Handler mHandler=new Handler()
    {
		...
		public void handleMessage(Message msg) {
			...
			super.handleMessage(msg);
			 //Log.i("test", "message what = " + msg.what);
	          if (msg.what == ANIMATION_OUT && isSTART){
	        	  go2Show(ANIMATION_OUT);
	          }else if(msg.what == ANIMATION_IN && isSTART)
	          {
    ...
    private void init() {  
        ...
        random = new Random();  
        vecKeywords = new ArrayList<PushNews> ();
    ...
    public boolean feedKeyword(List<PushNews> keyword) {  
    ...
    private boolean show() { 
    	//System.out.println("width:"+width+"  |  height:"+height+"  |  enableShow:"+enableShow+"  |  vecKeywords.size():"+vecKeywords.size());
        if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {  
            ...
            for (int i = 0; i < size; i++) {  
                String keyword = vecKeywords.get(i).getPushNews_title();  
                // 随机颜色  
...
//                txt.setBackgroundColor(Color.RED);
    ...
    public List<PushNews> getKeywords() {  
        return vecKeywords;  
    }  
    ...
    public void settype(){
    	...
    	TimerTask task = new TimerTask(){   
    		public void run() {   
    	    	...
    	    	int ran = random.nextInt(vecKeywords.size());
    	    	//System.out.println("ran:"+ran);
    	    	if(ran%2==0)
    	    	{
    	    		//rubKeywords();
    	    		 Message msg = mHandler.obtainMessage(ANIMATION_IN);
    	    		 msg.sendToTarget();
    	    		//go2Show(ANIMATION_IN);
    	    	}else
    	    	{
    	    		//rubKeywords();
    	    		Message msg = mHandler.obtainMessage(ANIMATION_OUT);
   	    		 	msg.sendToTarget();
    	    		//go2Show(ANIMATION_OUT);
    	    	}
    		}   
